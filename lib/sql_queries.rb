...
"SELECT users.name, users.age, SUM(pledges.amount) FROM pledges JOIN users ON users.id = pledges.user_id GROUP BY users.name ORDER BY users.name"
end
...
"SELECT projects.category, pledges.amounts FROM projects JOIN pledges ON pledges.project_id = projects.id HAVING projects.category = 'music'"
end
